...
AIntroSceneActor::AIntroSceneActor()
{
    ...
    if (PlaneMesh.Succeeded())
    {
        ...
        Plane->SetRelativeScale3D(FVector(12.0f, 9.0f, 1.0f)); // proporzioni 16:9
        Plane->SetRelativeRotation(FRotator(0.0f, 90.0f, 0.0f)); // ruota per adattarsi alla camera
    }
    ...
    Camera->SetRelativeLocation(FVector(0.0f, 0.0f, 800.0f));      // sopra il piano
    Camera->SetRelativeRotation(FRotator(-90.0f, 0.0f, 0.0f));     // guarda in basso
...
void AIntroSceneActor::BeginPlay()
{
    ...
    if (Camera)
    {
        Camera->Activate();  // Forza l'attivazione della camera
    }
